...
class IndexView(generic.ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        mens_results = RaceTime.objects.all().filter(event="8K", gender="Male", date__year=CURRENT_YEAR).order_by(
            "time")
        womens_results = RaceTime.objects.all().filter(event="6K", gender="Female", date__year=CURRENT_YEAR).order_by(
            "time")
        ...
        for i in range(len(mens_results)):
            if (len(mens_results[i].time) <= 8 and mens_results[i].firstName + " " + mens_results[
                i].lastName not in names):
                ranks.append(r)
                r += 1
        ...
        for i in range(len(womens_results)):
            if (len(womens_results[i].time) <= 8 and womens_results[i].firstName + " " + womens_results[
                i].lastName not in names):
                ranks.append(r)
                r += 1
        ...
        context["zipped_mens_8k"] = sorted(
            zip(final_mens_results[0], final_mens_results[1], final_mens_results[2], final_mens_results[3],
                final_mens_results[4]), key=lambda x: x[0], reverse=False)
        context["zipped_womens_6k"] = sorted(
            zip(final_womens_results[0], final_womens_results[1], final_womens_results[2], final_womens_results[3],
                final_womens_results[4]), key=lambda x: x[0], reverse=False)
...
class AthleteView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        athleteData = str(self.kwargs['name']).replace(';', '').replace("(", "").replace(")", "").replace('  ',
                                                                                                          ' ').split(
            " ")
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if fiveK_results != [[], [], [], [], []]:
            context["zipped_5k_results"] = sorted(
                zip(fiveK_results[0], fiveK_results[1], fiveK_results[2], fiveK_results[3], fiveK_results[4]),
                key=lambda x: x[2], reverse=True)

        if sixK_results != [[], [], [], [], []]:
            context["zipped_6k_results"] = sorted(
                zip(sixK_results[0], sixK_results[1], sixK_results[2], sixK_results[3], sixK_results[4]),
                key=lambda x: x[2], reverse=True)

        if eightK_results != [[], [], [], [], []]:
            context["zipped_8k_results"] = sorted(
                zip(eightK_results[0], eightK_results[1], eightK_results[2], eightK_results[3], eightK_results[4]),
                key=lambda x: x[2], reverse=True)
...
class MeetView(generic.ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        mens_results = RaceTime.objects.all().filter(gender="Male", meet=meetData[0], date__year=meetData[1]).order_by(
            "place")
        womens_results = RaceTime.objects.all().filter(gender="Female", meet=meetData[0],
                                                       date__year=meetData[1]).order_by("place")
        ...
        for i in range(len(allMensResults)):
            allMensResults[i] = zip(allMensResults[i][0], allMensResults[i][1], allMensResults[i][2],
                                    allMensResults[i][3], allMensResults[i][4], allMensResults[i][5], )
        ...
        for i in range(len(allWomensResults)):
            allWomensResults[i] = zip(allWomensResults[i][0], allWomensResults[i][1], allWomensResults[i][2],
                                      allWomensResults[i][3], allWomensResults[i][4], allWomensResults[i][5], )
...
class AllMeetsView(generic.ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["zipped_meets"] = sorted(zip(all_meets[0], all_meets[1], all_meets[2], all_meets[3]),
